...
const MENU: Record<string, Dish> = {
  ...
  "50":{"p":230000,"c":"rolls","n":{"ua":"Запечений з крабом","ru":"Запечённый с крабом","en":"Baked Crab","vn":"Cuộn nướng cua"}},
    ...
    try {
      if (SB_URL && SERVICE) {
        const ins = await fetch(`${SB_URL}/rest/v1/orders`, {
          ...
          body: JSON.stringify({
            customer_name: name, phone, address, comment, people, telegram: telegram || null,
